...
func set_up_position():
	game_position = "0".repeat(80)
	
	for i in 2:
		for j in 10:
			game_position[(i * 40 - 10) + j] = char(j + 49)
...
func move(move: Move):
	game_position[move.end] = game_position[move.start]
	game_position[move.start] = str(0)
...
func unmove(move: Move):
	game_position[move.start] = game_position[move.end]
	game_position[move.end] = str(0)
	values[move.end / 10] = calc_value(move.end)
	values[move.start / 10] = calc_value(move.start)
...
func calc_value(id: int):
	...
	for i in 10:
		for j in i:
			if (ord(game_position[zone + i]) - 48) + (ord(game_position[zone + j]) - 48) == 10 \
			and game_position[zone + i] != "0" and game_position[zone + j] != "0":
				value += 10
		
		value += ord(game_position[zone + i]) - 48
...
func print_pos():
	...
	for i in 8:
		if i != 3 and i != 7:
			out += game_position.substr(i * 10, 10).replace("0", " ") + " | " + str(values[i]) + "\n"
		else:
			out += game_position.substr(i * 10, 10).replace("0", " ") + " |\n"
		if i == 3:
			out += "-----------|---\n"
